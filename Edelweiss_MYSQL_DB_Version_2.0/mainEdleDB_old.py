...
if __name__ == '__main__':
    ...
    if status == True:
        ...
        # ows = cur.fetchall()
        #print(ows)
        engine = create_engine("mysql+pymysql://{user}:{pw}@localhost/{db},%urlquote('badpass')"
                               .format(user="root",
                                       pw="R@ting#2021",
                                       db="indexdb"))
        ...
        for i in range(num_theads):
            ...
            worker = Thread(target=objpEd.start, args=(q, results, isMarketON, diction, engine, conn))#  FolderIDs,
