...
class ProcessIn:
    ...
    def process(self, service, file_to_save, file_id, URL, PID, symbl, item, no_of_days, i, file, isMarketON=True):
        try:
            if isMarketON == True:
                    ...
                    for PID in pid:
                        ...
                        for i, item in enumerate(resolutions_list):
                            ...
                            if isDataAvailable == False:
                                ...
                                if resolution == 'W':
                                    if datetime.date.today().isoweekday() == 1:
                                        status = self.process(service, file_to_save, file_id, URL, PID, symbl, item, no_of_days, i, file, False)
                                        if status == False:
                                            print('No Data available in the given range of date')
                                if resolution == 'D':
                                    strcurrentDateTime = datetime.datetime.now(timezone('Asia/Calcutta')).strftime('%H:%M')
                                    strcurrentDateTime = strcurrentDateTime.replace(':', '.')
                                    if float(strcurrentDateTime) < float('10.15'):
                                        status = self.process(service, file_to_save, file_id, URL, PID, symbl, item, no_of_days, i, file, False)
                                        if status == False:
                                            print('No Data available in the given range of date')
                                if resolution == 5 or resolution == 15 or resolution == 30:
